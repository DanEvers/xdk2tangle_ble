	...
	void appInitSystem(void* cmdProcessorHandle, uint32_t param2)
{
	...
	taskCreated = xTaskCreate(bleSendData, "IOT2TANGLE BLE", TASK_STACK_SIZE_REQ, NULL, TASK_PRIO_REQ, &bleTaskHandle);
	...
	triggerRequestTimerHandle = xTimerCreate("triggerIOT2TANGLERequestTimer", (const TickType_t)atoi(INTER_REQUEST_INTERVAL) / portTICK_RATE_MS, pdFALSE, NULL, triggerRequestTimerCallback);
